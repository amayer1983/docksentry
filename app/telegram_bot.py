...
class TelegramBot:
    ...
    def _build_selfupdate_script(name, run_parts, image, stop_timeout=60,
        ...
        **ten seconds** and then sends SIGKILL — which is what killed
        ...
        # Already quoted by the caller, like run_parts. Empty for the
        # normal case where the image's own entrypoint is what runs.
    ...
    def _do_selfupdate(self, config, own_name, own_image, reply_to=None):
        ...
        # entrypoint puts its remaining tokens here, and a Cmd does as
        # well. Keeping the flag while discarding its arguments is worse
        # than doing neither — `--entrypoint python3` without the script
        # it should run gives a container that exits instantly and
        # restarts forever.

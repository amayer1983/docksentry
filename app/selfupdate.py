...
def swap(ctx, config, own_name, own_image, reply_to=None):
    ...
    # Retire our own past mistake without anybody having to do anything.
    # 2.17.10 and beta.27 shipped `/sbin/tini` as ENTRYPOINT, and their
    # self-update pinned that path onto whatever image came next — so a
    # container created by those two carries it forever, keeps needing
    # the compatibility shim, and would never pick up a real init later.
    #
    # A swap is a recreate anyway, so the cheapest place to put that
    # right is here: when the container's entrypoint is that path and the
    # target image does not ask for it, forget the container's and let
    # the image's own apply. One normal update and the container is back
    # on the ordinary entrypoint, silently.
    #
    # Scoped to the self-update on purpose. `_build_run_args` also
    # rebuilds other people's containers, and one of those may use tini
    # because its author chose to — that is theirs to keep. This path
    # only ever rebuilds Docksentry's own container, where the path got
    # there because we put it there.

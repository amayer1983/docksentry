...
| `DISCORD_GUILD_ID` | | Optional. Register the slash commands in one server instead of globally. Guild commands appear **instantly**, global ones can take up to an hour to propagate — so for a bot serving a single server, set this. Env-only. |

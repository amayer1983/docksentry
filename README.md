...
| `DOCKER_HOSTS` | | **Multi-host (experimental, unreleased).** Extra hosts this instance also manages, as `name:endpoint` pairs: `pve1:ssh://root@pve1, nas:tcp://nas:2375`. The machine Docksentry runs on is always managed and is *not* listed here — leave this unset and everything behaves exactly as a single-host install. Endpoints are whatever the container CLI accepts for `-H` (SSH or TCP / socket proxy); SSH relies on the CLI's own handling, so key-based login must already work non-interactively for the user Docksentry runs as. A host that can't be reached is reported and skipped — it won't stall the others. Self-update stays local-only: Docksentry updates the instance it runs in, not the ones on your other boxes. Env-only. |

...
- **A host endpoint may be `context://<name>`**, which runs `podman --connection <name>` or `docker --context <name>`. Measured, and the reason is worse than it looks: `podman --url ssh://…` takes the identity of the **default** connection whatever host it points at — not the one matching the URL — and it ignores `~/.ssh/config` entirely, where `docker -H ssh://` reads it because it shells out to the real `ssh`. So on Podman there was no way to give each host its own key. See `docs/podman.md`.
- **An unreachable host says why.** The Status page printed `nas: unreachable` for four different causes — wrong key, closed port, DNS, wrong socket path — while Telegram and Discord had always quoted the CLI. The CLI's own last line is shown now.

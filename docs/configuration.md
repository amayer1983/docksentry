...
Where the containers are, and which CLI talks to them.
...
The schedule, the policy, and the containers it applies to.
...
Credentials, mirrors, and certificates for pulling images.
...
Crash alerts, health flips, and the weekly report.
...
Reclaiming space, and warning before it runs out.
...
The Telegram bot: notifications and commands.
...
Two separate paths: a webhook that only posts, and a bot that also answers.
...
Each channel has a switch, so it can be silenced without clearing its settings. Also on the Connections page.
